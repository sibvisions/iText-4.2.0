...
package com.lowagie.mpl.text.html.simpleparser;
...
import com.lowagie.mpl.text.ElementTags;
import com.lowagie.mpl.text.ExceptionConverter;
import com.lowagie.mpl.text.FontProvider;
...
import com.lowagie.mpl.text.TextElementArray;
import com.lowagie.mpl.text.html.HtmlTags;
import com.lowagie.mpl.text.html.Markup;
import com.lowagie.mpl.text.pdf.PdfPTable;
import com.lowagie.mpl.text.pdf.draw.LineSeparator;
import com.lowagie.mpl.text.xml.simpleparser.SimpleXMLDocHandler;
import com.lowagie.mpl.text.xml.simpleparser.SimpleXMLParser;
	...
	public void startElement(String tag, HashMap h) {
		...
		try {
			...
			if (tag.equals(HtmlTags.UNORDEREDLIST)) {
				...
				skipText = true;
				cprops.addToChain(tag, h);
				com.lowagie.mpl.text.List list = new com.lowagie.mpl.text.List(false);
				try{
			...
			if (tag.equals(HtmlTags.ORDEREDLIST)) {
				...
				skipText = true;
				cprops.addToChain(tag, h);
				com.lowagie.mpl.text.List list = new com.lowagie.mpl.text.List(true);
				try{
					list.setIndentationLeft(new Float(cprops.getProperty("indent")).floatValue());
				}catch (Exception e) {
					list.setAutoindent(true);
		...
		try {
			...
			if (tag.equals(HtmlTags.UNORDEREDLIST)
				...
				if (stack.empty())
					return;
				Object obj = stack.pop();
				if (!(obj instanceof com.lowagie.mpl.text.List)) {
					stack.push(obj);
					return;
				}
				if (stack.empty())
			...
			if (tag.equals(HtmlTags.LISTITEM)) {
				...
				if (!(list instanceof com.lowagie.mpl.text.List)) {
					stack.push(list);
					return;
				}
				ListItem item = (ListItem) obj;
				((com.lowagie.mpl.text.List) list).add(item);
