...
import com.lowagie.mpl.text.pdf.internal.PolylineShape;
...
*/
    ...
    private boolean drawImage(Image img, Image mask, AffineTransform xform, Color bgColor, ImageObserver obs) {
        ...
        try {
            com.lowagie.mpl.text.Image image = null;
            if(!convertImagesToJPEG){
                image = com.lowagie.mpl.text.Image.getInstance(img, bgColor);
            }
            else{
                ...
                scaled.flush();
                scaled = null;
                image = com.lowagie.mpl.text.Image.getInstance(baos.toByteArray());
                
            }
            if (mask!=null) {
                com.lowagie.mpl.text.Image msk = com.lowagie.mpl.text.Image.getInstance(mask, null, true);
                msk.makeMask();
    ...
    private void setPaint(boolean invert, double xoffset, double yoffset, boolean fill) {
        if (paint instanceof Color) {
            ...
            try {
                ...
                com.lowagie.mpl.text.Image image = com.lowagie.mpl.text.Image.getInstance(img, null);
