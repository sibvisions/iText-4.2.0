...
public class PdfPKCS7 {
        ...
        while (true) {
            ...
            for (int k = 0; k < seq.size(); ++k) {
                ...
                if (seq.getObjectAt(k) instanceof ASN1TaggedObject) {
                    ...
                    if (tag.isExplicit()) {
                    	obj = tag.getExplicitBaseObject().toASN1Primitive();
                    }
                    else {
                    	obj = tag.getBaseObject().toASN1Primitive();
                    }
    ...
    public PdfPKCS7(byte[] contentsKey, String provider) {
        try {
            ...
            DERTaggedObject tag = (DERTaggedObject)signedData.getObjectAt(1);
            ...
            ASN1Sequence content = (ASN1Sequence)obj;
            ...
            if (rsaData.size() > 1) {
            	...
            	tag = (DERTaggedObject)rsaData.getObjectAt(1);
                ...
                DEROctetString rsaDataContent = (DEROctetString)obj;
                ...
                for (int k = 0; k < sseq.size(); ++k) {
                    ...
                    if (((ASN1ObjectIdentifier)seq2.getObjectAt(0)).getId().equals(ID_MESSAGE_DIGEST)) {
                        ...
                        for (int j = 0; j < seqout.size(); ++j) {
		                    ...
		                    if (tg.isExplicit()) {
		                    	obj = tg.getExplicitBaseObject().toASN1Primitive();
		                    }
		                    else {
		                    	obj = tg.getBaseObject().toASN1Primitive();
		                    }
                            
                            ASN1Sequence seqin = (ASN1Sequence)obj;
                            findOcsp(seqin);
                        }
                    }
                }
        ...
        try {
            for (Enumeration aliases = keystore.aliases(); aliases.hasMoreElements();) {
                try {
                    ...
                    ContentVerifierProvider cp = new JcaContentVerifierProviderBuilder().setProvider(provider).build(certStoreX509.getPublicKey());
                    
                    if (ocsp.isSignatureValid(cp))
                        return true;
                }
                catch (Exception ex) {
                }
        ...
        try {
            for (Enumeration aliases = keystore.aliases(); aliases.hasMoreElements();) {
                try {
                    ...
                    SignerInformationVerifier siv = new JcaSignerInfoVerifierBuilder(new JcaDigestCalculatorProviderBuilder().build()).setProvider(provider).build(certStoreX509);
                    
                    ts.isSignatureValid(siv);
                    return true;
                }
                catch (Exception ex) {
                }
    ...
    public boolean isRevocationValid() {
        ...
        try {
            ...
            DigestCalculatorProvider digCalcProv = new JcaDigestCalculatorProviderBuilder().setProvider(provider == null ? "BC" : provider).build();
            
            CertificateID tis = new CertificateID(digCalcProv.get(CertificateID.HASH_SHA1), new JcaX509CertificateHolder(isscer), sigcer.getSerialNumber());
     ...
     * StringTokenizer.
     */
    public static class X500NameTokenizer {
        private String          oid;
        private int             index;
